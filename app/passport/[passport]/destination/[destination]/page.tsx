...
export async function generateStaticParams() {
  // Return union of TOP_PAIRS_300 + PRIMARY_CLUSTER_PAIRS (deduplicated)
  return getAllStaticPairs();
}
...
export async function generateMetadata({ params }: Props): Promise<Metadata> {
  ...
  // Enhanced titles for primary cluster pairs
  let title: string;
  if (isInPrimaryCluster && visaRule) {
    const shortPassport = passport.name === 'United States' ? 'U.S.' : passport.name;
    const visaTypeLower = visaRule.visa_type.toLowerCase();

    if (visaTypeLower.includes('visa_free')) {
      const stayText = visaRule.max_stay_days ? ` – ${visaRule.max_stay_days} Days Visa-Free` : ' – Visa-Free';
      title = `${destination.name} Visa Requirements for ${shortPassport} Citizens${stayText} (2026)`;
    } else if (visaTypeLower === 'visa_required') {
      title = `${destination.name} Visa Requirements for ${shortPassport} Citizens – Visa Required (2026)`;
    } else if (visaTypeLower === 'evisa') {
      title = `${destination.name} Visa Requirements for ${shortPassport} Citizens – eVisa (2026)`;
    } else if (visaTypeLower === 'visa_on_arrival') {
      title = `${destination.name} Visa Requirements for ${shortPassport} Citizens – Visa on Arrival (2026)`;
    } else {
      title = buildTitle({ type: 'pair', passport: passport.name, destination: destination.name });
    }

    // Ensure title doesn't exceed 65 characters
    if (title.length > 65) {
      title = `${destination.name} Visa for ${shortPassport} Citizens (2026) | VisaInfoGuide`;
    }
  } else {
    title = buildTitle({ type: 'pair', passport: passport.name, destination: destination.name });
  }
      ...
      <main className="min-h-screen bg-gray-50">
        <div className="max-w-5xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
          ...
          <h1 className="text-4xl font-bold text-gray-900 mb-6">
            {destination.name} visa requirements for {passport.name} citizens
          </h1>
