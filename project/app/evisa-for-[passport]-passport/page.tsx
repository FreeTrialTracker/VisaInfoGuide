...
const getEvisaDestinations = unstable_cache(
  async (dbSlug: string) => {
  ...
  ['evisa-destinations'],
  { revalidate: 86400, tags: ['evisa-destinations'] }
);
...
export async function generateMetadata({ params }: PageProps): Promise<Metadata> {
  ...
  const title = `eVisa Countries for ${meta.nationality} Passport (2026) | VisaInfoGuide`;
  const description = `Complete list of countries ${meta.nationality} passport holders can visit with an eVisa in 2026. Apply online before you travel — no embassy visit required.`;
  const url = canonicalUrl(`/evisa-for-${params.passport}-passport`);
...
export default async function EvisaForPassportPage({ params }: PageProps) {
  const meta = SEO_SLUG_TO_DB[params.passport];
  if (!meta) notFound();

  const destinations = await getEvisaDestinations(meta.dbSlug);
        ...
        dangerouslySetInnerHTML={{
          __html: JSON.stringify({
            ...
            itemListElement: [
              ...
              { '@type': 'ListItem', position: 3, name: `${meta.nationality} Passport`, item: `https://visainfoguide.com/evisa-for-${params.passport}-passport` },
            ],
          }),
        }}
      />
        ...
        <div className="container mx-auto px-4 py-8 max-w-5xl">
          <Breadcrumbs
            items={[
              ...
              { name: `${meta.nationality} Passport`, url: `/evisa-for-${params.passport}-passport` },
            ]}
          />
          ...
          <header className="mt-8 mb-10">
            <div className="flex items-center gap-4 mb-6">
              ...
              <h1 className="text-4xl md:text-5xl font-bold text-gray-900 leading-tight">
                eVisa Countries for {meta.nationality} Passport (2026)
              </h1>
            </div>

            <div className="bg-blue-50 border-l-4 border-blue-500 p-5 rounded-r-lg">
              <p className="text-gray-800 leading-relaxed">
                <strong>{meta.nationality} passport holders can apply for an eVisa to {destinations.length} countries.</strong>{' '}
                Apply online before you travel — no embassy visit required. You receive your visa electronically before departure.
              </p>
            </div>
          </header>
          ...
          {destinations.length === 0 ? (
            <div className="bg-gray-50 border border-gray-200 rounded-lg p-12 text-center">
              ...
              <p className="text-gray-600 text-lg">No eVisa destinations found for this passport.</p>
            </div>
          ) : (
            <>
              <div className="grid grid-cols-3 sm:grid-cols-4 md:grid-cols-6 gap-2 mb-8 text-center text-sm">
                {activeRegions.map(([region, list]) => (
                  <a
                    key={region}
                    href={`#${region.toLowerCase().replace(' ', '-')}`}
                    className="bg-white border border-gray-200 rounded-lg px-2 py-2 hover:border-blue-300 hover:bg-blue-50 transition-colors"
                  >
                    <span className="font-medium text-gray-700 text-xs">{region}</span>
                    <p className="text-blue-600 font-bold">{list.length}</p>
                  </a>
                ))}
              </div>
                    ...
                    {list.map((d) => {
                      ...
                      return (
                        <Link
                          ...
                          className="group"
                        >
                          <Card className="transition-all duration-150 hover:shadow-md hover:border-blue-200">
                            <CardContent className="p-4 flex items-center justify-between">
                              ...
                              <ArrowRight className="w-3.5 h-3.5 text-gray-400 group-hover:text-blue-500 group-hover:translate-x-1 transition-all flex-shrink-0" />
                            </CardContent>
                          </Card>
                        </Link>
                      );
          ...
          <div className="bg-amber-50 border border-amber-200 rounded-lg p-5 mb-12">
            <p className="text-sm text-amber-800">
              <strong>Note:</strong> An approved eVisa does not guarantee entry. Border officers retain the right to deny admission. Always verify current requirements with official government sources before booking travel.
            </p>
          </div>
          ...
          <section className="mb-8">
            ...
            <div className="grid gap-3">
              ...
              <Link href={`/visa-free-countries-for-${params.passport}-passport`} className="group">
                <Card className="transition-all duration-200 hover:shadow-md hover:border-blue-200">
                  <CardContent className="flex items-center justify-between p-5">
                    <div>
                      <p className="font-semibold text-gray-900 group-hover:text-blue-700 transition-colors">
                        Visa-Free Countries for {meta.nationality} Passport
                      </p>
                      <p className="text-sm text-gray-500">No visa needed at all</p>
                    </div>
                    <ArrowRight className="w-4 h-4 text-gray-400 group-hover:text-blue-600 group-hover:translate-x-1 transition-all flex-shrink-0" />
                  </CardContent>
                </Card>
                ...
                <Card className="transition-all duration-200 hover:shadow-md hover:border-blue-200">
                  <CardContent className="flex items-center justify-between p-5">
                    <div>
                      ...
                      <p className="text-sm text-gray-500">All visa types for all destinations</p>
                    </div>
                    <ArrowRight className="w-4 h-4 text-gray-400 group-hover:text-blue-600 group-hover:translate-x-1 transition-all flex-shrink-0" />
                  </CardContent>
                </Card>
