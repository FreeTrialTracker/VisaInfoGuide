...
import { Globe, CircleCheck as CheckCircle, Circle as XCircle, FileText, ArrowRight, Clock, TriangleAlert as AlertTriangle, Shield, Wallet, Plane } from 'lucide-react';
...
export async function generateMetadata({ params }: PageProps): Promise<Metadata> {
  const destinationSlug = params.slug.replace(/-\d{4}$/, '');
  ...
  const description = data.metaDescription
    ?? `Complete ${data.name} entry requirements guide for 2026. Visa policies, passport validity rules, required documents, stay duration limits, and overstay penalties for all nationalities.`;
...
export default function DestinationEntryPage({ params }: PageProps) {
  ...
  const genericFaqs = [
    ...
    },
    {
      ...
      answer: `Entry to ${data.name} requires a valid passport ${data.passportValidityRequired.toLowerCase()}${data.proofOfFundsRequired ? ', proof of sufficient funds' : ''}${data.onwardTicketRequired ? ', proof of onward travel' : ''}${data.healthInsuranceRequired ? ', and valid health insurance' : ''}. Additional documents may include accommodation bookings, invitation letters, or return flight tickets.`,
    },
  ];

  const faqs = data.customFaqs ?? genericFaqs;

  const officialSources = data.officialSources ?? [
    `${data.name} Ministry of Foreign Affairs`,
    `${data.name} Immigration Bureau`,
    `${data.name} Embassy and Consulate Announcements`,
      ...
      <div className="min-h-screen bg-gradient-to-b from-gray-50 to-white">
        <div className="container mx-auto px-4 py-8 max-w-5xl">
          ...
          <article className="prose prose-lg max-w-none">
            ...
            {data.highlightCards && data.highlightCards.length > 0 && (
              <section id="visa-options" className="mb-12">
                <h2 className="text-3xl font-bold text-gray-900 mb-6">Visa Options Overview</h2>
                <div className="grid gap-6">
                  {data.highlightCards.map((card) => (
                    <Card key={card.title} className={`border-l-4 ${card.borderColor ?? 'border-teal-500'}`}>
                ...
                <div className="flex items-start gap-4">
                  ...
                  <div>
                    ...
                    <p className="text-gray-800 leading-relaxed">
                      <strong>Never overstay your authorized period.</strong> Even brief overstays create permanent records that affect all future international travel. If you need to extend your stay, apply for an extension before your current authorization expires.
                    </p>
                  </div>
                </div>
              </div>
