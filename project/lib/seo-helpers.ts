import { PRERENDER_PRIORITY_PAIRS } from './countries';
import { RequirementStatus, PassportValidityRequirement } from './supabase';

/**
 * Returns true if a pair is in the prerender priority list.
 * This is a signal for link prominence and sitemap priority weighting only.
 * It does NOT gate whether a pair page can exist — all valid database pairs
 * render on-demand via ISR regardless of this value.
 */
export function isPriorityPair(passportSlug: string, destinationSlug: string): boolean {
  return PRERENDER_PRIORITY_PAIRS.some(
    p => p.passport === passportSlug && p.destination === destinationSlug
  );
}

/** @deprecated Use isPriorityPair instead */
export const isCuratedPair = isPriorityPair;
