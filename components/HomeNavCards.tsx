...
const cards = [
  ...
  },
  {
    ...
    links: [
      ...
      { label: 'Browse by Destination', href: '/resources#browse-by-destination' },
  ...
  },
  {
    ...
    icon: Calculator,
    accent: 'from-amber-50 to-white border-amber-100 hover:border-amber-400',
              ...
              <ul className="mt-auto space-y-1.5 pt-4 border-t border-gray-200">
                {card.links.map((link) => (
                  <li key={link.href}>
                    <Link
                      href={link.href}
                      className="flex items-center gap-1.5 text-sm font-medium text-gray-700 hover:text-teal-700 transition-colors group/link"
                    >
                      <ArrowRight className="w-3.5 h-3.5 text-gray-400 group-hover/link:text-teal-500 transition-colors shrink-0" />
                      {link.label}
                    </Link>
                  </li>
                ))}
                <li>
                  <Link
                    href={card.href}
                    className="flex items-center gap-1 text-sm font-semibold text-teal-600 hover:text-teal-800 transition-colors mt-2"
                  >
                    View all {card.label}
